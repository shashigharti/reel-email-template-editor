...
export default function Editor() {
  ...
  useEffect(() => {
    ...
    fetchSettings("default")
      .then((response) => {
        const setting = response?.data?.settings?.use_editor;
        if (setting != "undefined") {
          setUseEditor(setting === "true");
        }
      })
  ...
  function setEditorUsage(newValue) {
    saveSettings("default", {
      use_editor: newValue === true ? "true" : "false",
    });
    setUseEditor(newValue);
  }

  return (
    <div
      className="reel-email-template-editor"
      ...
      <div style={{ display: "flex", gap: "20px" }}>
        <div style={{ flex: 1 }}>
          ...
          <div style={{ marginTop: "10px" }}>
            ...
            />
            <HelpText
              info="Send a test email using the currently selected template. Please note that the data used 
        ...
        >
          ...
          />
          <HookList
            onAttach={(hID) => setHookID(hID)}
            selectedHookID={hookID}
            hooks={hooks}
          />
          <UserTypeList
            onSelect={(userTypeID) => setSelectedUserTypeID(userTypeID)}
